...
#             conv1 = tf.nn.avg_pool(conv1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='VALID')
            ...
            # #  增加LSTM
            # lstm_model = tf.keras.layers.LSTMCell(50)
            # l9, self.previous_state_c = lstm_model(l2, self.previous_state_c)

            # w9 = tf.Variable(tf.truncated_normal(
            #     [50, 1], stddev=0.01), name='w2')
            # bias9 = tf.Variable(tf.constant(
            #     0.0, shape=[1], dtype=tf.float32), name='b2')
            # self.v = tf.nn.relu(tf.matmul(l9, w9) + bias9)


            #todo  DNC
            dnc = DNC(input_size=50, output_size=1,
                      seq_len=0, num_words=10, word_size=32, num_heads=1)
            self.v = tf.reshape(dnc.run(l2), [-1, np.shape(dnc.run(l2))[-1]])   #todo LSTM  l2
        ...
        with tf.variable_scope('loss'):
            ...
            if METHOD['name'] == 'kl_pen':
                ...
                # print('debug!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
                # print(surr, ratio,self.tfadv,BELTA,l2_loss_a)
    ...
    def _build_anet(self, name, trainable):
        with tf.variable_scope(name):
            ...
            mu = 1 * tf.nn.sigmoid(tf.matmul(l4, w6) + bias6)         #todo change from LATM(l5) to l4
            ...
            sigma = self.decay * \
                tf.nn.sigmoid(tf.matmul(l4, w7) + bias7) + 0.00001    #todo change from LATM(l5) to l4
    ...
    def choose_action(self, s, dec):
        ...
        return np.clip(a[0], 0.001, 1)  # clip the output   return np.clip(a[0], 0.0001, 1)
